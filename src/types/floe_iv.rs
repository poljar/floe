...
// would like to configure and would have `Into` implementations for `usize` as well as `u32`.
...
///
/// This initialization vector is randomly generated at the start of the encryption operation.
...
impl<const N: usize> FloeIv<N> {
    ...
    ///
    /// This should only be used when we're parsing an existing [`FloeIv`] from a bytestring.
